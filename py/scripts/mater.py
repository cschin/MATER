...
os.system("echo {} > reads.lst".format(sys.argv[1]))
os.system("shmr_mkseqdb -d reads.lst -p reads >> /dev/null")
...
mer2HLAtype = pickle.load(open("../data/mmer2HLAtype.pickle", "rb"))
read_sdb = SequenceDatabase("reads.idx", "reads.seqdb")
...
for allele in sorted(allele_weight.keys()):
    ...
    print("#", allele, len(allele2reads[allele]))
    for t, w in c[:10]:
        #print(allele2reads[t])
        c = list(allele_weight2[t].items())
        c.sort(key=lambda _:-_[1])
        t2, w2 =  c[0]
        print(allele, t, "%0.2f" % w, allele_count[allele][t], t2)
    print("#")

for allele in sorted(allele_weight.keys()):
    if allele not in ("A", "B", "C"):
        continue
    f = open("reads_{}.fa".format(allele),"w")
